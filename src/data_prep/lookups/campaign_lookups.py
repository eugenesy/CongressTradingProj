...
class CampaignLookup:
    def __init__(self, events_path=None):
        self.events_path = events_path or config.CAMPAIGN_FINANCE_EVENTS_PATH
    ...
    def get_edges(self, timestamp: pd.Timestamp) -> pd.DataFrame:
        ...
        # Read file in memory-safe chunks
        for batch in pf.iter_batches(batch_size=5_000_000):
            ...
            agg = past_events.groupby(['bioguide_id', 'industry_code'], observed=True).agg(
                total_donation_amount=('weight', 'sum'),
                donation_count=('donation_count', 'sum'),
                latest_donation_date=('date', 'max')
            ).reset_index()
        ...
        final_agg = final_df.groupby(['bioguide_id', 'industry_code'], observed=True).agg(
            total_donation_amount=('total_donation_amount', 'sum'),
            donation_count=('donation_count', 'sum'),
            latest_donation_date=('latest_donation_date', 'max')
        ).reset_index()
        
        final_agg['event_type'] = 'DONATION'
        return final_agg