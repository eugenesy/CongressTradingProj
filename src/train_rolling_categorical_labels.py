        ...
        for month in range(1, 13):
            ...
            for epoch in range(1, max_epochs + 1):
                ...
                for batch in train_loader:
                    ...
                    # 1. Price Embedding (Current Batch)
                    # Handle potential missing price sequence if dataloader sliced weirdly
                    if hasattr(batch, 'price_seq'):
                        price_seq = batch.price_seq
                    else:
                        # Fallback (should typically not happen if data built correctly)
                        price_seq = torch.zeros((len(src), 14), device=device)
                    
                    # Encode current price context for the predictor
                    price_emb = model.get_price_embedding(price_seq) # [Batch, 32]
                    
                    # Augment the message for Memory Update (so memory stores price context)
                    ...
                    # 3. History / Edge Features Construction
                    # We need to construct edge features for the GNN using historical interactions
                    if len(e_id) > 0:
                        hist_t = data.t[e_id]
                        hist_msg = data.msg[e_id] # This is the RAW message (3 dims)
                        
                        # Hist Price Emb (we need to compute this on the fly for history too)
                        ...
                        # Masked label: show actual if resolved, else 0.5 (neutral placeholder)
                        ...
                        # Final edge_attr construction
                        # [TimeEnc(100) + RawMsg(3) + PriceEmb(32) + MaskedLabel(1) + Age(1)] = ~137 dims
                        edge_attr = torch.cat([rel_t_enc, hist_msg, hist_price_emb, masked_label, age_feat], dim=-1)
                    else:
                        # Handle rare case of no neighbors
                        # Dims: time_dim + raw_msg + price_emb + label + age
                        # 100 + 3 + 32 + 1 + 1 = 137
                        feature_dim = model.time_dim + model.augmented_msg_dim + 2 # augmented includes price
                        edge_attr = torch.zeros((0, feature_dim), device=device)
                    ...
                    with torch.no_grad():
                        ...
                        y = batch.y.long().cpu().numpy()
                        
                        epoch_preds.extend(preds_cls)
                        epoch_targets.extend(y)
                    
                    # 6. Update Memory State for next batch
                ...
                if len(epoch_targets) > 0:
                    # FIX 3: Use 'weighted' average and remove binary thresholding
                ...
                # Note: Validation data is strictly chronological. We must update memory 
                # as we go so the graph state is correct for subsequent batches.
                for batch in val_loader:
                    ...
                    # 3. History / Edge Features Construction (Must match Train Loop)
                    if len(e_id) > 0:
                        hist_t = data.t[e_id]
                        ...
                        # Fallback for empty history (rare)
                        edge_attr = torch.zeros((0, model.augmented_msg_dim + model.time_dim + 2), device=device)
                    ...
                    with torch.no_grad():
                        ...
                        # FIX: Multiclass Prediction (Argmax instead of Sigmoid)
                        p_cls = logits.argmax(dim=1).cpu().numpy()
                        val_preds.extend(p_cls)
                        val_targets.extend(batch.y.long().cpu().numpy())
                    
                    # 5. Update State
                    model.memory.update_state(src, dst, t, augmented_msg)
                    neighbor_loader.insert(src, dst)
                
                # Calculate Validation F1 (Weighted for Multiclass)
                val_f1 = 0.0
            ...
            for batch in test_loader:
                ...
                if len(e_id) > 0:
                    ...
                    age_seconds = (batch_max_t - hist_t).float()
                    age_feat = torch.log1p(age_seconds / 86400.0).unsqueeze(-1)
                ...
                with torch.no_grad():
                    ...
                    # FIX: Use Argmax for Multiclass Predictions
                    p_cls = logits.argmax(dim=1).cpu().numpy()
                    y = batch.y.long().cpu().numpy()
            ...
            try:
                ...
                print(f"  [RESULT] {year}-{month:02d}: ACC={acc:.4f} | F1={f1:.4f} | Macro-F1={macro_f1:.4f} | Count={count}")
                ...
                axes[0].set_title(f'{year}-{month:02d}: Loss')
                
                axes[1].plot(range(1, len(epoch_train_f1s)+1), epoch_train_f1s, 'g-o')
                axes[1].set_title(f'{year}-{month:02d}: Train F1')
                
                axes[2].plot(range(1, len(all_batch_losses)+1), all_batch_losses, 'r-', linewidth=0.5)
                axes[2].set_title(f'{year}-{month:02d}: Batch Loss')
...
if __name__ == "__main__":
    ...
    # Retrieve number of classes from the saved map AND DELETE IT
    if hasattr(data, 'label_map'):
        NUM_CLASSES = len(data.label_map)
        print(f"Detected Multiclass Task: {NUM_CLASSES} classes")
        del data.label_map  # <--- CRITICAL: Remove dict so slicing doesn't crash
    else:
        NUM_CLASSES = 1
        print("Detected Binary/Regression Task (Defaulting to 1 output)")
