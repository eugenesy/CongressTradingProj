...
def train_and_evaluate(data, df_filtered, num_nodes, num_parties, num_states, num_classes,
                       window_months=24, step_months=1, max_epochs=20, patience=5):
    ...
    # This fulfills your requirement: "Predict N binary labels"
    # ---------------------------------------------------------
    def decode_to_binary(class_indices, num_thresholds):
        ...
        # Class 0 (Fails all) -> >0? False.
        # Class 1 (Passed T1) -> >0? True. >1? False.
        return (class_indices > threshold_indices).float()
    # ---------------------------------------------------------

    # Date Alignment
    start_date = df_filtered['Traded'].min()
    end_date = df_filtered['Traded'].max()
    
    # We start the first test window after 'window_months' of data
    current_test_start = start_date + pd.DateOffset(months=window_months)
    ...
    while current_test_start < end_date:
        ...
        test_end = current_test_start + pd.DateOffset(months=step_months)
        gap_start = current_test_start - pd.DateOffset(months=1)
        gap_end = current_test_start
        
        print(f"\n=== RETRAINING For Window: {current_test_start.strftime('%Y-%m')} ===")
        
        # 1. SPLIT INDICES (Expanding Window Fix)
        # Use ALL history prior to gap_start
        train_mask = (df_filtered['Traded'] < gap_start)
        gap_mask = (df_filtered['Traded'] >= gap_start) & (df_filtered['Traded'] < gap_end)
        test_mask = (df_filtered['Traded'] >= current_test_start) & (df_filtered['Traded'] < test_end)
        ...
        if len(test_idx) == 0:
            print(f"No trades in {current_test_start.strftime('%Y-%m')}, skipping.")
        ...
        def get_edge_attr(n_id, edge_index, e_id, batch_t):
            ...
            # Replace -100 (sanitize result) with -1 for feature embedding purposes
            # (or any neutral value). Here we use -1.0 as "Unknown"
        ...
        if len(targets_binary_all) > 0:
            ...
            print(f"  [RESULT] {current_test_start.strftime('%Y-%m')}: BinarySubsetAcc={acc:.4f} | WeightedF1={f1:.4f} | Count={count}")
            print("-" * 50)
            print(classification_report(final_t, final_p, zero_division=0))
            
            results.append({
                "Window": current_test_start.strftime('%Y-%m'),
                "Accuracy": acc,
                "F1": f1,
                "Count": count
            })
