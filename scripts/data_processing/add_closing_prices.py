...
from scripts.utils import load_checkpoint, save_checkpoint, load_csv_with_path, save_csv_with_path

# CONFIGURATION
INPUT_CSV = '../data/v5_transactions_with_benchmark.csv'
HIST_PKL = '../data/all_tickers_historical_data.pkl'
...
def add_closing_prices_to_transactions(
    ...
    for col in ['Close_TradeDate', 'Close_1Month', 'Close_3Months', 'Close_6Months']:
        if col not in df.columns:
            df[col] = np.nan

    print("Processing transactions and adding closing prices...")
    ...
    save_csv_with_path(df, output_csv, index=False)
    save_checkpoint(processed_indices, checkpoint_file)
    print("All rows processed. Final file saved as", output_csv)
    return df
