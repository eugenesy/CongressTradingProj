...
def _create_monthly_labels(row):
    ...
    labels = {'Label_1M': np.nan, 'Label_3M': np.nan, 'Label_6M': np.nan}
    ...
    for period in ['1M', '3M', '6M']:
        ...
        elif transaction in ['Sale', 'Sale (Partial)', 'Sale (Full)']:
            labels[f'Label_{period}'] = 0 if excess_above_6 else 1
        else:
            labels[f'Label_{period}'] = np.nan
