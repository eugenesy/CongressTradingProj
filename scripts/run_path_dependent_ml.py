...
def run_ml_comparison_year(df, test_year):
    ...
    num_feats = [c for c in ['Filing_Gap', 'Late_Filing', 'In_SP100'] if c in df.columns]
    ...
    for target in ['Label_Median', 'Label_Q3']:
        ...
        results_list.append({
            ...
            'auc_id': auc_id, 'auc_all': auc_all,
            'p10_id': p10_id, 'p10_all': p10_all
        })
        
    res_df = pd.DataFrame(results_list)
    out_dir = Path("results/metrics")
    out_dir.mkdir(parents=True, exist_ok=True)
    res_df.to_csv(out_dir / f"xgb_res_{test_year}.csv", index=False)
    print(f"Saved results to {out_dir}/xgb_res_{test_year}.csv")
