...
\> \*\*Missing:\*\* Eugene, can you specify which versions of the raw data files the user should save, and where, before they run the build script?

Before running the preprocessing pipeline, ensure you have sourced the necessary raw transaction data.

1. \*\*Download Raw Data\*\*: Obtain the required transaction datasets (e.g., Senate/House disclosures).
2. \*\*Storage\*\*: Save the files into the \`data/raw/\` directory.
3. \*\*Validation\*\*: Ensure filenames match the expected schema defined in the configuration.
